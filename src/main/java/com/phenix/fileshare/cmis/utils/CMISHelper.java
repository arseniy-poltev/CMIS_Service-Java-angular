...
public class CMISHelper {
    ...
    public static CmisObject getCmisObject(final Session session,
                                           ...
                                           final String what) throws CMISException {
        if (session == null) {
            throw new IllegalArgumentException("Session must be set!");
        }

        if (id == null || id.length() == 0) {
            throw new CMISException("Invalid id for " + what + "!");
        }
        ...
        try {
            ...
            throw new CMISException("The " + what + " does not exist!",
                    onfe);
        } catch (CmisBaseException cbe) {
            throw new CMISException("Could not retrieve the " + what
                    + ":" + cbe.getMessage(), cbe);
        }
    }
    ...
    public static Document getDocumet(final Session session,
        ...
        if (!(cmisObject instanceof Document)) {
            throw new CMISException("Object is not a document!");
        }
    ...
    public static Folder getFolder(final Session session,
        ...
        if (!(cmisObject instanceof Folder)) {
            throw new CMISException("Object is not a folder!");
        }
    ...
    public static CmisObject getCmisObjectByPath(final Session session,
        ...
        if (path == null || !path.startsWith("/")) {
            throw new CMISException("Invalid path to " + what + "!");
        }
